...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    //主状态机
    while (((m_check_state == CHECK_STATE_CONTENT) && (line_status == LINE_OK)) || ((line_status = parse_line()) == LINE_OK)) //读一行
    {
        text = get_line();
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    //如果是注册或者登录的话, post方式
    else if (cgi == 1 && (strcasecmp(p, "log") == 0 || strcasecmp(p, "register") == 0))
    {
        ...
        else
        {
            ...
            {
                strcpy(m_real_url, "/welcome.html");
            }
            else
            {
...
bool http_conn::write()
{
    ...
    while (1)
    {
        ...
        bytes_to_send -= temp;
        printf("temp: %d, m_iv[1].iov_base: %p, bytes_have_send: %d, m_write_idx: %d\n", m_iv[1].iov_base, bytes_have_send, m_write_idx);
        //如果已经把响应行和响应头写出去，接下来只需要继续写mmap文件的内容
        if (bytes_have_send >= m_iv[0].iov_len)
        {
            m_iv[0].iov_len = 0; 
            //bytes_have_send - m_write_idx表示除了write_buf，文件已经写了多少东西
...
bool http_conn::process_write(HTTP_CODE ret)
{
    ...
    //除了正常的打开文件和404，其他都只写write_buf里的东西
    m_iv[0].iov_base = m_write_buf;
    m_iv[0].iov_len = m_write_idx;
    m_iv_count = 1;
